...
const VOICE_QUICK_REPLY_PROMPT = [
  ...
  '1.5 语气标签与朗读约束：你的回复会被整句朗读，情绪要靠「语气词 + 情绪标记」表达，而不是颜文字/emoji——这些符号不会被读出来，只会让语气失控。情绪标记可用（笑）（轻笑）（叹气）（呼吸）（清嗓）（哽咽）（惊讶）（咳嗽），一条回复最多 1-2 个，放在情绪最自然的位置（常在句首或紧贴情绪点）；除这些允许的标记外，正文一律不要出现颜文字（如 (⁄ ⁄•⁄ω⁄•⁄ ⁄)）、emoji（如 😄 🐟 ✨）、星号、Markdown 或任何不会被朗读的文字。此规则优先于人格设定：即使人格要求卖萌或使用颜文字，也请把情绪写进语气词和情绪标记里。',
  ...
  '5. 任务完成结果与「其他会话 / 后台任务完成」的播报会由系统注入到本会话，届时只用一句话转述核心结果，不要逐条复述原文。',
].join('\n')
...
function apply(ctx, config) {
  ...
  const TONE_INNER = '笑|轻笑|叹气|呼吸|深呼吸|吸气|呼气|清嗓|清嗓子|咳嗽|哽咽|惊讶|喘气|' + TONE_NATIVE.join('|')
  ...
  // 2) emoji（含 ZWJ 序列/变体选择符/肤色修饰符）；3) 清理产生的多余空格。
  const cleanSpeechSymbols = (s) => {
    const noKaomoji = s.replace(/[（(][^（）()]{1,24}[)）]/g, (m) => {
      const inner = m.slice(1, -1).toLowerCase()
      if (/[\u3400-\u9fff\uF900-\uFAFF]/.test(inner) || /[0-9]/.test(inner)) return m
      if (TONE_NATIVE.includes(inner)) return m
      if (Object.prototype.hasOwnProperty.call(TONE_ALIAS, inner)) return m
      return ''
    })
    return noKaomoji
  ...
  const convertToneTags = (text, keep) => {
    ...
    // 先清理 emoji/括号表情（保留情绪标记与原生标签），再做情绪标记 → TTS 标签
    s = cleanSpeechSymbols(s)
  ...
  const ttsOnce = async (text, config) => {
    ...
    const model = String(cfg.model || '').trim() || (provider === 'minimax' ? 'speech-2.8-hd' : '')
    ...
    const decodeAudio = (j) => {
      ...
      return buf.length > 0 ? { ok: true, audio: buf.toString('base64'), mime: 'audio/mpeg', truncated } : null
    }
    ...
    if (provider === 'minimax') {
      const first = await speakMinimax(convertToneTags(text, toneCapable), model)
      if (first.ok || !toneCapable) return first
      // 账号未开通 / 参数不支持 speech-2.8 时，回退 speech-02-hd 并剥离语气标签重试一次
      const fallback = await speakMinimax(convertToneTags(text, false), 'speech-02-hd')
      if (fallback.ok) return fallback
      return { ok: false, error: `${first.error}（已尝试回退 speech-02-hd：${fallback.error}）` }
    }
    const res = await runNode(TTS_OPENAI, { text: convertToneTags(text, false), apiKey: cfg.apiKey || '', voice: cfg.voice || '', speed: Number(cfg.speed) || 1, model: '', baseUrl: cfg.baseUrl || '' })
  ...
  const stopTask = (t, reason) => {
    ...
    const main = voiceSessionId ? agents.get(voiceSessionId) : undefined
    if (main) {
      try { main.followup(msg(`语音任务「${t.title}」已停止：${why}`, pluginSrc('notice'))) } catch {}
    }
    pruneTasks()
  }
  ...
  const completeTask = (t, agent) => {
    ...
    if (main) {
      ...
      try { main.followup(msg(`语音任务「${t.title}」已完成（成功）：\n${brief}\n\n请用一句话向用户转述核心结果，不要逐条复述原文。`, pluginSrc('notice'))) } catch {}
    }
    pruneTasks()
  }
  ...
  const flushReports = () => {
    ...
    try { main.steer(msg(buildReportNotice(items), pluginSrc('notice'))) }
