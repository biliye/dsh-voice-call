...
const VOICE_QUICK_REPLY_PROMPT = [
  ...
  '1.5 语气标签与朗读约束：你的回复会被整句朗读，情绪要靠「语气词 + 情绪标记」表达，而不是颜文字/emoji——这些符号不会被读出来，只会让语气失控。情绪标记可用（笑）（轻笑）（叹气）（呼吸）（清嗓）（哽咽）（惊讶）（咳嗽），一条回复最多 1-2 个，放在情绪最自然的位置；多句回复时不要只在句首放一个——承载结论或收尾的那一句也要带上语气词或情绪标记，否则后半句会被念得很平、很生硬；只能用上面列出的标记，不要自创——没有对应实现的标记（如（轻声）（温柔））会被逐字念成"轻声""温柔"，想表达轻声、温柔、认真这类语气就用语气词和短句节奏，不要写标记；除这些允许的标记外，正文一律不要出现颜文字（如 (⁄ ⁄•⁄ω⁄•⁄ ⁄)）、emoji（如 😄 🐟 ✨）、星号、Markdown 或任何不会被朗读的文字。此规则优先于人格设定：即使人格要求卖萌或使用颜文字，也请把情绪写进语气词和情绪标记里。',
...
function apply(ctx, config) {
  ...
  // 2) 「朗读方式」标记（TONE_STRIP，如（轻声）（温柔））直接删除——没有原生标签可映射，
  //    留着就会被逐字念出来；
  ...
  const cleanSpeechSymbols = (s) => {
    const noKaomoji = s.replace(/[（(][^（）()]{1,24}[)）]/g, (m) => {
      ...
      if (TONE_STRIP.includes(inner)) return ''
  ...
  const ttsOnce = async (text, config) => {
    ...
    if (provider === 'minimax') {
      const firstText = convertToneTags(text, toneCapable)
      const first = await speakMinimax(firstText, model)
      note(model, firstText, first)
    ...
    const openaiText = convertToneTags(text, false)
    const res = await runNode(TTS_OPENAI, { text: openaiText, apiKey: cfg.apiKey || '', voice: cfg.voice || '', speed: Number(cfg.speed) || 1, model: '', baseUrl: cfg.baseUrl || '' })
    note('openai-compatible', openaiText, { ok: !!(res.ok && res.audio), error: res.error })
