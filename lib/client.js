...
`
		...
		function Fab(props) {
			...
			const refs = React.useRef({ ws: null, stream: null, actx: null, proc: null, recorder: null, chunks: [], settings, sessionId: null, since: 0, pollBusy: false, vad: null, vadBusy: false, ttsUntil: 0, pendingPcm: [] })
							...
							if (res?.ok && res.events?.length) {
								...
								for (const ev of res.events) {
									if (ev.kind === 'reply') {
										...
										if (s.ttsEnabled) {
											setStatus('回复中…')
											const estMs = Math.max(2500, (ev.text || '').length * 160 + 1500)
											call(API.tts, { text: ev.text, config: ttsConfig(s) })
												.then((t) => { if (t?.ok && t.audio) playAudio(t.audio, t.mime || 'audio/mpeg', t.truncated, estMs) })
												.catch(() => { refs.current.ttsUntil = 0; setStatus('聆听中…') })
										} else setStatus('聆听中…')
			...
			const playAudio = (b64, mime, truncated, estMs) => {
				try {
					...
					refs.current.ttsUntil = Date.now() + (estMs || 4000) + 600
					a.onended = () => { if (Date.now() > refs.current.ttsUntil - 600) refs.current.ttsUntil = 0; setStatus('聆听中…') }
					a.onerror = () => { refs.current.ttsUntil = 0; setStatus('聆听中…') }
					a.play().catch(() => { refs.current.ttsUntil = 0; setStatus('聆听中…') })
			...
			const transcribePcm = async (pcm, s) => {
				...
				try {
					...
					const config = isFunasrHttp
						? { baseUrl: s.funasrHttpBase, apiKey: s.funasrHttpKey, model: s.funasrHttpModel || 'fun-asr-nano' }
			...
			const startVadMic = async () => {
				...
				const vad = { state: 'idle', pcm: [], speechChunks: 0, silenceChunks: 0 }
				...
				proc.onaudioprocess = (ev) => {
					...
					const st = r.settings
					if (!r.vad) return
					if (Date.now() < r.ttsUntil) { r.vad.state = 'idle'; r.vad.pcm = []; r.vad.speechChunks = 0; r.vad.silenceChunks = 0; return }
					...
					if (rms > thr) {
						if (r.vad.state !== 'speaking') { r.vad.state = 'speaking'; r.vad.pcm = []; r.vad.speechChunks = 0; r.vad.silenceChunks = 0; setStatus('聆听中…（正在说话）') }
						r.vad.pcm.push(buf)
						r.vad.speechChunks++
				...
				open ? e('div', { className: 'va-panel', style: panelPos },
					...
					e('div', { className: 'va-body' },
						tab === 'call' ? e(Fragment, null,
							...
							: e('div', { className: 'va-hint' }, '点击「开始通话」后直接说话，停顿 1.2 秒即自动识别发送并语音回复。所有语音内容保存在专属工作区「语音通话」的专属会话中，本体重启后保持同一会话；耗时任务自动分发给其他会话执行。'),
