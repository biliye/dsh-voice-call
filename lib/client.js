...
`
		...
		function Fab(props) {
			...
			const ttsConfig = (s) => ({ provider: s.ttsProvider, apiKey: s.ttsKey, groupId: s.ttsGroupId, voice: s.ttsVoice, speed: s.ttsSpeed, maxChars: s.ttsMaxChars, baseUrl: effectiveBase(s), model: s.ttsProvider === 'minimax' ? String(s.ttsModel || '').trim() : '' })
						...
						tab === 'call' ? e(Fragment, null,
							...
							e('div', { className: 'va-set' },
								...
								settings.ttsProvider === 'minimax' ? e(Fragment, null,
									...
									e('input', { className: 'va-input', value: settings.ttsMinimaxBase, onChange: (ev) => set('ttsMinimaxBase', ev.target.value), placeholder: 'https://api.minimax.chat/v1/t2a_v2' }), e('input', { className: 'va-input', value: settings.ttsModel, onChange: (ev) => set('ttsModel', ev.target.value), placeholder: '模型 speech-2.8-hd（默认，支持语气标签）' }),
									settings.ttsMinimaxBase && !/^https?:\/\//.test(settings.ttsMinimaxBase) ? e('div', { className: 'va-hint' }, '⚠ 域名需以 https:// 开头，否则使用默认国内版域名。') : null,
									e('div', { className: 'va-hint' }, 'MiniMax 必须同时填写 API Key 与 GroupId（platform.minimaxi.com → 账户管理 → 接口密钥），两者必须来自同一分组，否则报 token not match group。国内版默认域名 api.minimax.chat；国际版（minimaxi.com）填 https://api.minimaxi.com/v1/t2a_v2。语音助手会按语气在正文加入情绪标记（如（笑）（叹气）），朗读时转为语气：MiniMax 需 speech-2.8 系列模型支持（默认 speech-2.8-hd，账号不支持时自动回退 speech-02-hd 并去掉标记），OpenAI 兼容 TTS 不支持标记、会自动剥离只读正文。'),
								) : e(Fragment, null,
