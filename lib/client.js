window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = {
			...
			ttsMinimaxKey: '',
			ttsMimoBase: 'https://api.xiaomimimo.com/v1/chat/completions',
			ttsMimoKey: '',
			// OpenAI 兼容那家沿用不加前缀的 ttsKey（与 ttsBase / ttsModel / ttsVoice 同一组）
			ttsKey: '',
			ttsGroupId: '',
			ttsVoice: '',
			ttsSpeed: 1,
			...
			ttsMimoModel: '',
			ttsMimoFormat: 'wav',
			// MiMo 音色克隆（mimo-v2.5-tts-voiceclone）的样本：填 .mp3/.wav 路径；
			// 留空则用 Host 数据目录 $DSH_HOME/voice-call/voice-clone/ 下的样音
			ttsCloneSample: '',
			// 导演模式：off=不发演绎指令；fixed=用下面的固定剧本；auto=每轮从回复里解析【角色/场景/指导】
			directorMode: 'off',
			directorScript: '',
			ttsMaxChars: 300,
			persona: '',
		...
		// TTS 的 API Key 按提供商各存一份：三家的 Key 互不通用，共用一个字段的后果是
		// 「切了提供商还得重填 Key」——重填写回的是同一个字段，会把上一家覆盖掉，来回切等于每次都要重填。
		// 下面是「提供商 → Key 字段名」映射的唯一所有者：设置页的输入框与发请求的 config 都从它取，
		// 任何一处单独写死 s.ttsKey 都会让两边不一致（界面看着对、真正发出去的仍是旧 Key）。
		const ttsKeyField = (s) => (s.ttsProvider === 'minimax' ? 'ttsMinimaxKey' : s.ttsProvider === 'mimo' ? 'ttsMimoKey' : 'ttsKey')
		const loadSettings = () => {
			let merged = { ...DEFAULTS }
			try {
				...
				if (raw) {
					...
					// 一次性迁移（本次改动之前只有单个 ttsKey，三家共用）：把旧 Key 归位到当时
					// 所用提供商的专属字段，否则老用户升级后请求里的 apiKey 会变空、朗读直接失败。
					// 判据是「新字段在存档里从未出现过」，迁移一次后该判据不再成立。
					// 归属只能按当前提供商推断——旧模型不区分，这正是那个缺陷留下的代价。
					if (stored.ttsMinimaxKey === undefined && stored.ttsMimoKey === undefined) {
						const field = ttsKeyField(merged)
						if (field !== 'ttsKey' && merged.ttsKey) merged[field] = merged.ttsKey
					}
				}
			} catch {}
		...
		// 语音助手的回复可带情绪标记（如（笑）（叹气））：面板展示时剥离标记只显示正文
		// （朗读时由 Host 端按 TTS 提供商转换/剥离，见 lib/index.js convertToneTags）
		const stripTone = (text) => String(text || '').replace(/[（(]\s*(?:笑|轻笑|叹气|呼吸|深呼吸|吸气|呼气|清嗓|清嗓子|咳嗽|哽咽|惊讶|喘气|laughs|chuckle|sighs|breath|inhale|exhale|clear-throat|coughs|crying|pant|gasps|emm|humming|groans|applause)\s*[)）]/gi, '')
		const sessionTitle = (list, id) => {
			const row = id ? list?.byId?.[id] : undefined
			return row?.displayTitle || row?.title || id || '未绑定会话'
		}
		const STYLE = `
.va-fab { position: fixed; width: 56px; height: 56px; border-radius: 50%; color: #fff; cursor: pointer; z-index: 2147483000; user-select: none; touch-action: none; border: none; padding: 0; background: transparent; transition: transform .18s ease; }
.va-fab:hover { transform: scale(1.07); }
.va-fab-glass { position: absolute; inset: 0; border-radius: 50%; overflow: hidden; display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,.34); background: radial-gradient(circle at 30% 26%, rgba(255,255,255,.5), rgba(255,255,255,.06) 44%), radial-gradient(circle at 72% 74%, rgba(150,120,255,.32), rgba(255,255,255,0) 56%), linear-gradient(135deg, rgba(118,92,224,.30), rgba(84,160,255,.22)); backdrop-filter: blur(14px) saturate(160%); -webkit-backdrop-filter: blur(14px) saturate(160%); box-shadow: 0 8px 24px rgba(80,70,160,.35), inset 0 1px 2px rgba(255,255,255,.5), inset 0 -8px 14px rgba(60,50,140,.28); transition: box-shadow .3s ease, border-color .3s ease; }
.va-fab-glass::before { content: ''; position: absolute; inset: 6%; border-radius: 50%; background: radial-gradient(circle at 30% 24%, rgba(255,255,255,.62), rgba(255,255,255,0) 48%); pointer-events: none; }
.va-fab-glass::after { content: ''; position: absolute; inset: 12%; border-radius: 50%; border: 1px solid rgba(255,255,255,.24); box-shadow: inset 0 0 0 3px rgba(255,255,255,.05); pointer-events: none; }
.va-fab-aura { position: absolute; inset: -3px; border-radius: 50%; opacity: 0; pointer-events: none; background: conic-gradient(from 0deg, #7c5cff, #00d4ff, #3dffd0, #ff6bd6, #ff9d5c, #7c5cff); filter: blur(7px) saturate(150%); animation: va-aurora 3.2s linear infinite; }
.va-fab.on .va-fab-glass { border-color: rgba(255,255,255,.55); }
.va-fab.on .va-fab-aura { opacity: .9; }
.va-fab-icon { position: relative; z-index: 2; display: flex; align-items: center; justify-content: center; filter: drop-shadow(0 1px 2px rgba(0,0,0,.25)); }
.va-fab.on .va-fab-icon { animation: va-icon-breathe 1.8s ease-in-out infinite; }
.va-fab-halo { position: absolute; inset: -7px; border-radius: 50%; pointer-events: none; opacity: 0; border: 2px solid rgba(140,110,255,.55); }
.va-fab.on .va-fab-halo { animation: va-halo 2.2s ease-out infinite; }
@keyframes va-aurora { to { transform: rotate(360deg); } }
@keyframes va-icon-breathe { 0%,100% { transform: scale(1); } 50% { transform: scale(1.08); } }
@keyframes va-halo { 0% { transform: scale(.82); opacity: .85; } 100% { transform: scale(1.5); opacity: 0; } }
...
.va-badge.stopped { background: #d6303155; color: #ff7675; }
...
`
		...
		// 悬浮球麦克风图标（Feather mic）：替代 emoji，观感更精致
		const MicIcon = (props) => e('svg', Object.assign({ viewBox: '0 0 24 24', width: 26, height: 26, fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round' }, props),
			e('path', { d: 'M12 2a3 3 0 0 0-3 3v6a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z' }),
			e('path', { d: 'M19 10v1a7 7 0 0 1-14 0v-1' }),
			e('line', { x1: '12', y1: '18', x2: '12', y2: '22' }),
			e('line', { x1: '8', y1: '22', x2: '16', y2: '22' }),
		)
		let ctxRef = null
		function Fab(props) {
			...
			// 唤醒词模式休眠时用户依赖语音、并不在看屏幕，清空 viewing 以放行所有完成播报
			React.useEffect(() => {
				const asleep = settings.wakeMode && wake === 'asleep'
				const cur = asleep ? null : (list?.current || null)
				call(API.viewing, { sessionId: cur }).catch(() => {})
			}, [list?.current, settings.wakeMode, wake])
			...
			React.useEffect(() => { call(API.setAnnounce, { enabled: settings.announceOtherDone !== false }).catch(() => {}) }, [settings.announceOtherDone])
			// 把朗读提供商 / 导演模式同步给 Host：专属会话的朗读规则提示词要跟着变
			// （例如 MiMo 才能用的中文风格标签与导演模式指令格式）。失败不影响朗读本身，
			// 因为每次 TTS 请求都自带完整配置。
			React.useEffect(() => {
				call(API.ttsSettings, { provider: settings.ttsProvider, directorMode: settings.ttsProvider === 'mimo' ? settings.directorMode : 'off' }).catch(() => {})
			}, [settings.ttsProvider, settings.directorMode])
			React.useEffect(() => {
						...
						.then((res) => {
							if (res?.ok && res.events?.length) {
								...
								for (const ev of res.events) {
									if (ev.kind === 'reply') {
										setMsgs((m) => [...m.slice(-19), { role: 'assistant', text: stripTone(ev.text) }])
										if (s.ttsEnabled) {
											// 朗读文本必须**原样**交给 Host：导演指令块的「分离 → 压缩正文 → 标签转换」
											// 全在 Host 的 prepareSpeech 里做。这里若先折叠换行或按字数截断，
											// 「三段标记 + --- + 正文」的结构会被压坏，Host 解析不出来就只能整段跳过朗读
											// （实测事故：回复本身格式完全正确，却一个字都不念）。
											const speak = String(ev.text || '')
											setStatus('回复中…')
											...
											if (urlErr) {
												...
												call(API.tts, { text: speak, config: ttsConfig(s) })
													.then((t) => {
														if (t?.ok && t.audio) {
															// 播放时长/静音窗口按 Host 回传的「实际朗读正文字数」估算：
															// 用原始回复长度会把导演指令块算进去，麦克风被静音过久。
															const spoken = Number(t.sentChars) > 0 ? Number(t.sentChars) : speak.length
															playAudio(t.audio, t.mime || 'audio/mpeg', t.truncated, Math.max(2500, spoken * 160 + 1500))
														}
														else {
															refs.current.ttsUntil = 0
															const err = String(t?.error || '未知错误').slice(0, 200)
															setStatus('语音回复失败: ' + err)
															setMsgs((m) => [...m.slice(-19), { role: 'notice', text: '⚠ 语音回复失败: ' + err + '（请在设置页检查 TTS 提供商 / Key / GroupId / 克隆样本）' }])
														}
													})
										...
										// 记录到「动态」页签即可；语音播报已改由 Host 合并后 steer 进主会话回复（避免双播）
										const item = { sessionId: String(ev.sessionId || ''), text: String(ev.text || '').slice(0, 300), title: String(ev.title || ''), time: ev.time || Date.now() }
			...
			const effectiveBase = (s) => {
				const raw = String((s.ttsProvider === 'minimax' ? s.ttsMinimaxBase : s.ttsProvider === 'mimo' ? s.ttsMimoBase : s.ttsBase) || '').trim()
				try { new URL(raw); return raw } catch { return s.ttsProvider === 'minimax' ? 'https://api.minimax.chat/v1/t2a_v2' : s.ttsProvider === 'mimo' ? 'https://api.xiaomimimo.com/v1/chat/completions' : 'https://api.openai.com/v1/audio/speech' }
			}
			// 每次朗读都带上完整的朗读配置（含导演模式）——Host 端是「每请求独立配置」，
			// 不依赖之前同步过的设置，避免同步失败时用错提供商/剧本
			const ttsConfig = (s) => ({
				provider: s.ttsProvider,
				apiKey: String(s[ttsKeyField(s)] || ''),
				groupId: s.ttsProvider === 'minimax' ? s.ttsGroupId : '',
				voice: s.ttsProvider === 'mimo' ? (s.ttsVoice || 'mimo_default') : s.ttsVoice,
				speed: s.ttsSpeed,
				// maxChars 是「朗读正文」的字数预算：Host 先分离导演指令块，再按它压缩 --- 之后的正文。
				// 语音场景按一句话的体量给到 100 字上限；设置里的 ttsMaxChars 更小时听设置的。
				maxChars: Math.min(Number(s.ttsMaxChars) || 300, 100),
				baseUrl: effectiveBase(s),
				model: s.ttsProvider === 'minimax' ? String(s.ttsModel || '').trim() : s.ttsProvider === 'mimo' ? String(s.ttsMimoModel || '').trim() : '',
				format: s.ttsProvider === 'mimo' ? (s.ttsMimoFormat || 'wav') : 'mp3',
				directorMode: s.ttsProvider === 'mimo' ? (s.directorMode || 'off') : 'off',
				directorScript: s.directorScript || '',
				// 音色克隆样本路径（仅 model 命中 voiceclone 时 Host 才用它，留空即用插件默认样音）
				voiceSample: s.ttsCloneSample || '',
			})
			...
			const [stopTaskId, setStopTaskId] = React.useState(null)
			const stopTask = (taskId) => {
				setStopTaskId(taskId)
				call(API.taskStop, { taskId })
					.then((r) => {
						if (r?.ok) { setStatus('任务已停止'); refreshTasks() }
						else setStatus('停止失败: ' + (r?.error || ''))
					})
					.catch(() => setStatus('停止失败'))
					.finally(() => setStopTaskId(null))
			}
			...
			return e(Fragment, null,
				e('button', { className: 'va-fab' + (calling ? ' on' : ''), style: pos, ...dragHandlers, title: calling ? '语音通话中' : '语音通话助手', onClick: () => { if (refs.current.suppressClick) { refs.current.suppressClick = false; return } setOpen(!open) } },
					e('span', { className: 'va-fab-halo' }),
					e('span', { className: 'va-fab-glass' },
						e('span', { className: 'va-fab-aura' }),
						e('span', { className: 'va-fab-icon' }, e(MicIcon)),
					),
				),
				open ? e('div', { className: 'va-panel', style: panelPos },
					e('div', { className: 'va-head' },
						...
						e('button', { className: 'va-x', onClick: () => setOpen(false) }, '✕'),
					),
						...
						tab === 'call' ? e(Fragment, null,
							...
							tasks.length ? tasks.map((t) => e('div', { key: t.taskId, className: 'va-task' },
								e('div', { className: 'va-t' },
									...
									e('span', { className: 'va-badge ' + t.status }, t.status === 'running' ? '执行中' : t.status === 'completed' ? '已完成' : t.status === 'stopped' ? '已停止' : (t.status === 'failed' || t.status === 'error') ? '失败' : '等待'),
								),
								e('div', { className: 'va-hint' }, (t.failReason ? ((t.status === 'failed' ? '失败原因: ' : '') + t.failReason + '\n') : '') + (t.result || t.progress || '（暂无进度）').slice(0, 300)),
								e('div', { className: 'va-row' },
									t.status === 'running' ? e('button', { className: 'va-btn danger', disabled: stopTaskId === t.taskId, onClick: () => stopTask(t.taskId) }, stopTaskId === t.taskId ? '停止中…' : '⏹ 停止') : null,
							...
							otherDones.length ? otherDones.map((d, i) => e('div', { key: i, className: 'va-task' },
								e('div', { className: 'va-t' },
									e('strong', null, '「' + (d.title || sessionTitle(list, d.sessionId)) + '」'),
							...
							e('div', { className: 'va-set' },
								...
								e('select', { className: 'va-select', value: settings.ttsProvider, onChange: (ev) => set('ttsProvider', ev.target.value) },
									...
									e('option', { value: 'mimo' }, 'MiMo-V2.5-TTS'),
									e('option', { value: 'openai' }, 'OpenAI 兼容 TTS'),
								),
								settings.ttsProvider === 'openai' ? e(Fragment, null,
									...
									e('input', { className: 'va-input', type: 'password', value: settings[ttsKeyField(settings)], onChange: (ev) => set(ttsKeyField(settings), ev.target.value), placeholder: 'API Key（OpenAI 兼容）' }),
								) : e(Fragment, null,
									e('input', { className: 'va-input', type: 'password', value: settings[ttsKeyField(settings)], onChange: (ev) => set(ttsKeyField(settings), ev.target.value), placeholder: settings.ttsProvider === 'mimo' ? 'MiMo API Key（与其它提供商各存一份）' : 'MiniMax API Key（与其它提供商各存一份）' }),
									settings.ttsProvider === 'mimo' ? e(Fragment, null,
										e('input', { className: 'va-input', value: settings.ttsMimoBase, onChange: (ev) => set('ttsMimoBase', ev.target.value), placeholder: 'https://api.xiaomimimo.com/v1/chat/completions' }),
										settings.ttsMimoBase && !/^https?:\/\//.test(settings.ttsMimoBase) ? e('div', { className: 'va-hint' }, '⚠ 域名需以 https:// 开头，否则使用默认域名。') : null,
										e('input', { className: 'va-input', value: settings.ttsMimoModel, onChange: (ev) => set('ttsMimoModel', ev.target.value), placeholder: '模型 mimo-v2.5-tts（预置音色）/ mimo-v2.5-tts-voiceclone（音色克隆）' }),
										e('select', { className: 'va-select', value: settings.ttsMimoFormat, onChange: (ev) => set('ttsMimoFormat', ev.target.value) },
											e('option', { value: 'wav' }, '音频格式 wav（默认）'),
											e('option', { value: 'mp3' }, '音频格式 mp3'),
										),
										e('input', { className: 'va-input', value: settings.ttsCloneSample, onChange: (ev) => set('ttsCloneSample', ev.target.value), placeholder: '克隆样本路径（.mp3/.wav，留空用插件数据目录里的样音）' }),
										e('div', { className: 'va-hint' }, '模型填 mimo-v2.5-tts-voiceclone 时用它复刻音色。官方没有「注册 voice_id」这一步：样本随每次朗读一起发给 MiMo（≤10MB base64，仅 mp3/wav），所以样音越短越省时间。留空则用 $DSH_HOME/voice-call/voice-clone/ 下的样音。只应克隆你本人或已获明确授权的声音。'),
										e('div', { className: 'va-hint' }, 'MiMo 不需要 GroupId，只填 API Key 即可。语速没有独立参数，请写在导演剧本的【指导】里。'),
									) : e(Fragment, null,
										...
										e('input', { className: 'va-input', value: settings.ttsModel, onChange: (ev) => set('ttsModel', ev.target.value), placeholder: '模型 speech-2.8-hd（默认，支持语气标签）' }),
								...
								// ---------- 音色 ----------
								// MiMo 只接受官方预置音色 ID（填错会报错），所以用下拉而不是自由输入；
								// 但 voiceclone 模型没有「预置音色」这回事——音色完全由上面的克隆样本决定
								// （Host 会用样本 DataURL 覆盖 voice），这里就不该再摆一个会误导人的下拉。
								settings.ttsProvider === 'mimo' ? (/voiceclone/i.test(String(settings.ttsMimoModel || ''))
									? e('div', { className: 'va-hint' }, '当前模型是音色克隆（voiceclone）：音色由上方「克隆样本」决定，预置音色下拉在这里不生效。')
									: e('div', { className: 'va-row' },
										e('select', { className: 'va-select', value: settings.ttsVoice || 'mimo_default', onChange: (ev) => set('ttsVoice', ev.target.value) },
											e('option', { value: 'mimo_default' }, 'MiMo-默认（mimo_default）'),
											e('option', { value: '冰糖' }, '冰糖（中文·女）'),
											e('option', { value: '茉莉' }, '茉莉（中文·女）'),
											e('option', { value: '苏打' }, '苏打（中文·男）'),
											e('option', { value: '白桦' }, '白桦（中文·男）'),
											e('option', { value: 'Mia' }, 'Mia（英文·女）'),
											e('option', { value: 'Chloe' }, 'Chloe（英文·女）'),
											e('option', { value: 'Milo' }, 'Milo（英文·男）'),
											e('option', { value: 'Dean' }, 'Dean（英文·男）'),
										),
										e('span', { className: 'va-hint' }, '音色'),
									)) : e('div', { className: 'va-row' },
								...
								// ---------- 导演模式 ----------
								// 三态控件常驻显示（不再被提供商条件隐藏）：之前藏在 settings.ttsProvider === 'mimo'
								// 里，用 MiniMax 时整块都不渲染，用户根本找不到这个开关。剧本照常保存，
								// 但只有朗读端是 MiMo 时才真正生效（见 ttsConfig 的 directorMode 取值）。
								e('div', { className: 'va-set' },
									e('label', null, '导演模式' + (settings.ttsProvider === 'mimo' ? '（MiMo-V2.5-TTS）' : '')),
									e('select', { className: 'va-select', value: settings.directorMode || 'off', onChange: (ev) => set('directorMode', ev.target.value) },
										e('option', { value: 'off' }, '关闭：只朗读回复正文'),
										e('option', { value: 'fixed' }, '固定剧本：每次朗读都用下面的剧本演绎'),
										e('option', { value: 'auto' }, '每轮自动：助手每轮自带【角色/场景/指导】'),
									),
									settings.directorMode === 'fixed' ? e(Fragment, null,
										e('textarea', { className: 'va-input', style: { minHeight: 96, resize: 'vertical' }, value: settings.directorScript, onChange: (ev) => set('directorScript', ev.target.value), placeholder: '【角色】百年门阀岑家的现任大当家，性冷淡而极具威压的低音御姐\n【场景】在祠堂的阴影里，面对企图带她私奔的男人\n【指导】语速极慢，句间留长空白；实音重而硬，尾音带一丝气声' }),
										e('div', { className: 'va-hint' }, '【角色】【场景】【指导】三段会作为 role=user 的演绎指令发给 MiMo（本身不会被朗读），回复正文作为 role=assistant 朗读。语速、气息、重音都写进【指导】。'),
									) : null,
									settings.directorMode === 'auto' ? e('div', { className: 'va-hint' }, '助手会在回复最前面输出【角色】【场景】【指导】，再用 --- 分隔真正要朗读的正文；面板只显示正文，指令不会念出来。若某轮没按格式给出分隔符，该轮会跳过朗读而不是把指令念出来。') : null,
									settings.ttsProvider === 'mimo'
										? e('div', { className: 'va-hint' }, 'MiMo 原生支持（笑）（叹气）（东北话）（唱歌）等中文风格标签，开启导演模式后这些标记会被演绎而不是念出来。')
										: e('div', { className: 'va-hint' }, '⚠ 导演模式只在「TTS 模型」选 MiMo-V2.5-TTS 时生效：MiMo 走 chat/completions，演绎指令与朗读正文分两条消息；当前提供商没有这个指令通道，剧本会保存但不发送。'),
								),
								e('div', { className: 'va-row' },
							...
							e('div', { className: 'va-set' },
								...
								e('div', { className: 'va-hint' }, '设置保存在本机浏览器（localStorage），含 API Key 明文；TTS 的 API Key 按提供商分别保存，切换提供商不会互相覆盖；仅支持 https 或本机 http 地址。FunASR Server 新版默认无需 API Key。'),
